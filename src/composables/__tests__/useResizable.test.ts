...
describe('useResizable', () => {
  ...
  it('init should call resizable on interactable with default edges', () => {
    ...
    expect(options.edges).toEqual({ left: true, right: true, bottom: true, top: true });
  });
  ...
  it('should handle resize events correctly', () => {
    const { init, isResizing, position, resizeData } = useResizable(context, {});
    init();
    const options = resizableMock.mock.calls[0][0];
    const startEvent = { target: context.interactable.value?.target } as any;
    const moveEvent = {
  ...
  it('should call interactable.set when resizableOptions are set', () => {
    ...
    expect(setMock).toHaveBeenCalledWith(expect.objectContaining(newOptions));
  });
