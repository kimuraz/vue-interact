...
describe('useInteractContext', () => {
    ...
    it('should update element style when position changes', async () => {
        ...
        await nextTick();
        expect(
            (interactable.value?.target as HTMLElement).style.transform,
    ...
    it('should update element style when size changes', async () => {
        ...
        await nextTick();
        expect((interactable.value?.target as HTMLElement).style.width).toBe(
            '300px',
        );
        expect((interactable.value?.target as HTMLElement).style.height).toBe(
            '400px',
        );
    });
